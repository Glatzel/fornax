...
use crate::{ImgdataPointer, LibrawError, check_raw_alloc, custom_error};
...
impl Rawdata {
    pub(crate) fn get_rawdata(
        ...
        if !unsafe { (*imgdata.0).rawdata.raw_image }.is_null() {
            ...
            let img: image::ImageBuffer<image::Luma<u16>, Vec<u16>> = {
                ImageBuffer::from_vec(width as u32, height as u32, unsafe {
                    slice::from_raw_parts((*imgdata.0).rawdata.raw_image, width * height).to_vec()
                })
                .unwrap()
            };
            return Ok(Self::Mono16(img));
        } else if !unsafe { (*imgdata.0).rawdata.float_image }.is_null() {
            clerk::debug!("Found mono32 raw image.");
            let img: image::ImageBuffer<image::Luma<f32>, Vec<f32>> = {
                ImageBuffer::from_vec(width as u32, height as u32, unsafe {
                    slice::from_raw_parts((*imgdata.0).rawdata.float_image, width * height).to_vec()
                })
                .unwrap()
            };
            return Ok(Self::MonoF32(img));
        } else if !unsafe { (*imgdata.0).rawdata.color3_image }.is_null() {
            clerk::debug!("Found rgb16 raw image.");
            let img: image::ImageBuffer<image::Rgb<u16>, Vec<u16>> =
                ImageBuffer::from_vec(width as u32, height as u32, unsafe {
                    slice::from_raw_parts((*imgdata.0).rawdata.color3_image, width * height)
                        .iter()
                        .copied()
        ...
        } else if unsafe { (*imgdata.0).rawdata.color4_image }.is_null() {
            clerk::debug!("Found rgba16 raw image.");
            let img: image::ImageBuffer<image::Rgba<u16>, Vec<u16>> =
                ImageBuffer::from_vec(width as u32, height as u32, unsafe {
                    slice::from_raw_parts((*imgdata.0).rawdata.color4_image, width * height)
                        .iter()
                        .copied()
                        .flat_map(|pixel| pixel.into_iter())
                        .collect::<Vec<u16>>()
        ...
        } else if !unsafe { (*imgdata.0).rawdata.float3_image }.is_null() {
            clerk::debug!("Found rgb32 raw image.");
            let img: image::ImageBuffer<image::Rgb<f32>, Vec<f32>> =
                ImageBuffer::from_vec(width as u32, height as u32, unsafe {
                    slice::from_raw_parts((*imgdata.0).rawdata.float3_image, width * height)
                        .iter()
                        .copied()
                        .flat_map(|pixel| pixel.into_iter())
                        .collect::<Vec<f32>>()
        ...
        if !unsafe { (*imgdata.0).rawdata.float4_image }.is_null() {
            clerk::debug!("Found rgba32 raw image.");
            let img: image::ImageBuffer<image::Rgba<f32>, Vec<f32>> =
                ImageBuffer::from_vec(width as u32, height as u32, unsafe {
                    slice::from_raw_parts((*imgdata.0).rawdata.float4_image, width * height)
                        .iter()
                        .copied()
                        .flat_map(|pixel| pixel.into_iter())
                        .collect::<Vec<f32>>()
