...
use crate::{Libraw, Error, check_run};
...
impl Libraw {
    pub fn open_file(&self, fname: &Path) -> Result<&Self, Error> {
        check_run!(unsafe {
    ...
    fn _open_file_ex(&self) -> Result<&Self, Error> { todo!() }
    fn _open_wfile(&self) -> Result<&Self, Error> { todo!() }
    fn _openwfile_ex(&self) -> Result<&Self, Error> { todo!() }

    pub fn open_buffer(&self, buf: &[u8]) -> Result<&Self, Error> {
        check_run!(unsafe {
    ...
    pub fn unpack(&self) -> Result<&Self, Error> {
        check_run!(unsafe { libraw_sys::libraw_unpack(self.imgdata_ptr()) });
        Ok(self)
    }
    pub fn unpack_thumb(&self) -> Result<&Self, Error> {
        check_run!(unsafe { libraw_sys::libraw_unpack_thumb(self.imgdata_ptr()) });
        Ok(self)
    }
    fn _unpack_thumb_ex(&self) -> Result<&Self, Error> { todo!() }
}
#[cfg(test)]
mod test {
    use std::io::Read;
