use crate::{ImageSizes, Libraw, LibrawErrorKind, Rawdata, check_raw_alloc};

// region:Data Structure
impl Libraw {
    pub fn get_image_sizes(&self) -> Result<ImageSizes, LibrawErrorKind> {
        check_raw_alloc!(self.imgdata_ptr());
        ImageSizes::new(self.arc_imgdata_ptr())
    }

    pub fn get_rawdata(&self) -> Result<Rawdata, LibrawErrorKind> {
        check_raw_alloc!(self.imgdata_ptr());
